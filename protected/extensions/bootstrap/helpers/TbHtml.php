...
EOD;
            ...
            foreach ($items as $itemOptions) {
                if (is_string($itemOptions)) {
                    ...
                    if (TbArray::popValue('active', $itemOptions, false)) {
                        // BS2's CSS/JS keyed the active state off <li
                        // class="active">; BS5's tab JS (and its CSS) key it
                        // off the trigger link itself (<a class="nav-link
                        // active">) — without this, switching tabs updates
                        // which link LOOKS active but BS5's JS can't find the
                        // previously active trigger to deactivate, so the old
                        // pane never hides. Kept on the <li> too since other
                        // nav styling may still expect it there.
                        self::addCssClass('active', $itemOptions);
                        self::addCssClass('active', $itemOptions['linkOptions']);
                    }
    ...
    public static function menuLink($label, $url, $htmlOptions = array())
    {
        ...
        // Bootstrap 5's nav/navbar CSS keys off .nav-item/.nav-link (BS2 only
        // needed the parent .nav/.nav-pills); adding them here fixes every
        // TbNav call site (main navbar, Operations sidebar) at once.
        self::addCssClass('nav-item', $htmlOptions);
        $linkOptions = TbArray::popValue('linkOptions', $htmlOptions, array());
        self::addCssClass('nav-link', $linkOptions);
    ...
    protected static function normalizeTabs($tabs, &$panes, &$i = 0)
    {
        ...
        foreach ($tabs as $tabOptions) {
            ...
            if (!empty($items)) {
                // data-bs-toggle (BS5) instead of data-toggle (BS2) — BS5's JS
                // only wires up its dropdown/tab data-api on the "bs"-prefixed
                // attribute.
                ...
                if (TbArray::popValue('active', $tabOptions, false)) {
                    // BS5 renamed the fade-transition "visible" class from
                    // .in to .show.
                    self::addCssClass('active show', $paneOptions);
                }
    ...
    {
        // BS5's breadcrumb is an <ol>, and the "/" divider is generated by
        // CSS (.breadcrumb-item + .breadcrumb-item::before) — no more manual
        // divider markup between items like BS2's <span class="divider">.
        self::addCssClass('breadcrumb', $htmlOptions);
        $output = self::openTag('ol', $htmlOptions);
        foreach ($links as $label => $url) {
            if (is_string($label) || is_array($url)) {
                $output .= self::openTag('li', array('class' => 'breadcrumb-item'));
                $output .= self::link($label, $url);
                $output .= '</li>';
            } else {
                $output .= self::tag('li', array('class' => 'breadcrumb-item active', 'aria-current' => 'page'), $url);
            }
        }
        $output .= '</ol>';
        return $output;
    ...
    {
        if (!empty($items)) {
            // BS5's pagination is just <ul class="pagination"> directly —
            // no wrapping <div class="pagination"> like BS2 (that div's
            // class was actually meant for the wrapper, but under BS5 it's
            // useless dead weight and the .pagination-* size classes belong
            // on the <ul>).
            $listOptions = TbArray::popValue('listOptions', $htmlOptions, array());
            self::addCssClass('pagination', $listOptions);
            $size = TbArray::popValue('size', $htmlOptions);
            if (!empty($size)) {
                self::addCssClass('pagination-' . $size, $listOptions);
            }
    ...
    {
        // BS5 keys pagination styling off .page-item (li) / .page-link (a)
        // — BS2 didn't need these, it styled bare li/a under .pagination.
        self::addCssClass('page-item', $htmlOptions);
        $linkOptions = TbArray::popValue('linkOptions', $htmlOptions, array());
        self::addCssClass('page-link', $linkOptions);
